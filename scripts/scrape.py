...
def parse_table(table, year):
    ...
    for tr in data_rows:
        ...
        # The 2025 table has separate IND and OTH columns. Two cases:
        #
        # (a) Separate values: IND=x, OTH=y (different raw text) → sum x+y
        # (b) Merged cell:     IND=x, OTH=x (identical raw text, colspan=2)
        #     The RowSpanTracker duplicates the value across both columns, so
        #     we must use the value ONCE rather than summing it with itself.
        ind_val = None
        oth_val = None
        ind_raw = ""
        oth_raw = ""
        if "IND" in pv_cols and "OTH" in pv_cols:
            ...
            if ind_ci < len(cells) and oth_ci < len(cells):
                ind_raw = fix_encoding(cells[ind_ci]).strip()
                oth_raw = fix_encoding(cells[oth_ci]).strip()
                ind_val = parse_pct(cells[ind_ci])
                oth_val = parse_pct(cells[oth_ci])

        combine_ind_oth = (ind_val is not None or oth_val is not None)
        if combine_ind_oth:
            # If raw text is identical, it is a merged cell — use value once
            if ind_raw == oth_raw and ind_val is not None:
                combined = ind_val
            else:
